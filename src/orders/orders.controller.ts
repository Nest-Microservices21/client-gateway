...
import { ORDERS_SERVICE } from 'src/config/orders.config';
...
export class OrdersController {
  ...
  async findAll(@Query() paginationDTO: OrderPaginationDto) {
    return this.ordersClient.send({ cmd: 'orders.find_all' }, paginationDTO);
  }
  @Get(':id')
  async findOne(@Param('id', ParseIdPipe) id: number) {
    return this.ordersClient.send({ cmd: 'orders.find_one' }, { id }).pipe(
      catchError((error) => {
